...
**Workers Free subrequest budget:** Cloudflare Free allows **50 subrequests per cron invocation** (each D1 call + each `fetch()`). Production `[env.production.vars]` sets `SUBREQUEST_LIMIT_PER_INVOCATION=50`, reserves ~38 subrequests for the schedule phase (`SCHEDULE_SUBREQUEST_RESERVE`), and chunks graph/sync work via `MAX_EDGES_PER_JOB`, `D1_BATCH_SIZE`, and `SYNC_ADDRESSES_PER_JOB`. Trace apply and source sync use continuation job payloads (`traceEdgeIndex`, batched sync payloads). Workers Paid defaults to 10,000 subrequests per invocation. **10 ms CPU per cron tick** on Free remains tight; subrequest chunking addresses the 50-cap, not CPU.
