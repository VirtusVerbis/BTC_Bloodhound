...
| `node apps/indexer/dist/index.js add-hacker <addr> [--label …] [--source …] [--hack coldcard\|liquid] [--yes] [--remote]` | Upsert flagged hacker (default `source=ops`, default `--hack coldcard`) + enqueue backfill. When `--hack liquid` and `--source` is omitted, source defaults to `x`. Known `--source` values: `ops`, `admin`, `coldcardwatch`, `coldcard_sweep_watch`, `coldcard_hack_tracker`, `public_seed`, `local_config`, `x` (dropdown category). Unknown sources prompt for confirmation; pass `--yes` to skip (non-interactive). |
...
**CF Workers tier quota presets:** Set `CF_WORKERS_TIER=free|paid` in `wrangler.toml` `[env.production.vars]` (and `config/sidecar.env` when running `--remote`). Tier selects defaults for cron quota pacing, D1 daily limits, and subrequest caps. Resolution order per setting: explicit env var (e.g. `D1_WRITE_DAILY_LIMIT`) → tier-specific cron var (`CRON_QUOTA_UTILIZATION_PCT_FREE` / `_PAID`) → built-in preset. `CRON_QUOTA_UTILIZATION_PCT` overrides both tier cron vars when set. Daily D1 row and Workers request totals are tracked via `D1RowMeter` on Worker and sidecar paths only; `wrangler d1 execute` admin CLI ops are not counted.

| Setting | Free preset | Paid preset |
|---------|-------------|-------------|
| `cronQuotaUtilizationPct` | 80 | 100 |
| `D1_READ_DAILY_LIMIT` | 5M | ~833M (25B/mo ÷ 30) |
| `D1_WRITE_DAILY_LIMIT` | 100K | ~1.67M (50M/mo ÷ 30) |
| `WORKERS_REQUEST_DAILY_LIMIT` | 100K | 10M |
| `SUBREQUEST_LIMIT_PER_INVOCATION` | 50 | 1000 |

Paid daily limits are pacing budgets (spread monthly included usage across UTC days), not CF hard caps. `/api/sync/status` includes `cfWorkersTier` for verification. **Upgrade checklist:** subscribe to Workers Paid → set `CF_WORKERS_TIER=paid` → deploy → watch `wrangler tail` for `[cron] tick done` (fewer `quota_pace` skips) → tune `SUBREQUEST_LIMIT_PER_INVOCATION` or `D1_WRITE_DAILY_LIMIT` if needed.

**Workers Free subrequest budget:** Cloudflare Free allows **50 subrequests per cron invocation** (each D1 `prepare().run()` / `.all()` / `.first()`, each `d1.batch()`, and each outbound `fetch()` attempt including retries). Cron `subreq=` telemetry counts **all** D1 and fetch usage during the tick via central instrumentation on the D1 binding and `instrumentedFetch`. Production tier presets default `SUBREQUEST_LIMIT_PER_INVOCATION` to 50 (free) or 1000 (paid); the worker reserves subrequests for the schedule phase (`SCHEDULE_SUBREQUEST_RESERVE`) and chunks graph/sync work via `MAX_SUBREQUESTS_PER_JOB`, `MAX_EDGES_PER_JOB`, `D1_BATCH_SIZE`, and `SYNC_ADDRESSES_PER_JOB`. Trace apply and source sync use continuation job payloads (`traceEdgeIndex`, `traceEdgesFlat`, batched sync payloads). **After deploy**, observe real `sched=` / `work=` percentiles in `wrangler tail` for ~20 ticks before lowering `SCHEDULE_SUBREQUEST_RESERVE` (e.g. 38 → 15–20). `stop=subreq` means the tick budget was exhausted; `stop=jobs_cap` means `JOBS_PER_TICK` was reached with budget remaining. Workers Paid platform default is 10,000 subrequests per invocation.
...
- **Enqueue caps (per cron tick):** code defaults are `CRAWL_ENQUEUE_PER_CRON=3`, `DOWNSTREAM_POLL_ENQUEUE_PER_CRON=2`, `POLL_HACKER_ENQUEUE_PER_CRON=1` (round-robin). **Production (Workers Free)** pins lower caps in `wrangler.toml` `[env.production.vars]`: `CRAWL_ENQUEUE_PER_CRON=1`, `DOWNSTREAM_POLL_ENQUEUE_PER_CRON=1`, `HACKER_MAINTENANCE_EVERY_N_CRONS=10`, `BALANCE_REFRESH_INTERVAL_SEC=900`, `DOWNSTREAM_POLL_INTERVAL_SEC=1200`.
- **Source sync deltas:** a content-hash change diffs against `addresses` before chunking. Only new or unflagged (promote) collectors and missing victims/vaults are applied. Known flagged hackers are not re-upserted or polled; `poll_hacker_address` stays on `maintainOneHacker`. Hack Tracker hashes the address set only (timestamp-only snapshot bumps are a no-op).
- **Queue latch (moderate backlog):** when pending depth reaches `MAX_QUEUE_DEPTH` (360), `queueSchedulingPaused` blocks new crawl/maint/expand enqueue. Latch clears when depth drops to `QUEUE_SCHEDULING_RESUME_DEPTH` (180), not only at zero. `QUEUE_DRAIN_FIRST_DEPTH` (60) and `QUEUE_SOFT_THROTTLE_DEPTH` (120) gate drain-first ticks and crawl/downstream-poll enqueue throttling. Soft throttle also skips hacker-maint `poll_hacker_address`, `refresh_live_balance`, and `audit_hacker_backfill`; backfill resume may still enqueue under `MAX_PENDING_BACKFILL_GLOBAL`.
- **Expand caps:** `MAX_PENDING_EXPAND_PER_ADDRESS` (2) and `MAX_PENDING_EXPAND_GLOBAL` (40) limit pending+running expand jobs; blocked enqueue defers with `[job] defer reason=expand_cap`. Crawl expand is hop-1 downstream only — flagged hacker history uses `backfill_hacker_address`.
- **Backfill cap:** `MAX_PENDING_BACKFILL_GLOBAL` (default `3`) limits pending+running `backfill_hacker_address` seeds. Continuations still enqueue. Overflow new hackers stay `expand_status=pending`; schedule refills one under the cap per tick. CLI `add-hacker` bypasses the cap.
- **Latch + continuations:** when latch is on, `expand_downstream` continuations are blocked (`reason=queue_paused`); `backfill_hacker_address` continuations may still enqueue to finish started backfills. At cap edge when not paused, ingest continuations may still enqueue one more job to finish in-flight work.
- **Poll time-slice:** every `POLL_SLICE_EVERY_N_CRONS` ticks (default 4), slot 0 skips ingest pick and claims the next poll/expand job (`CRON_EXPAND` priority 5, below `poll_hacker` 6).
...
- **Phase 2 (steady-state):** when backlog is stable, relax production caps toward `CRAWL_ENQUEUE_PER_CRON=2`, `BALANCE_REFRESH_INTERVAL_SEC=600`, `DOWNSTREAM_POLL_INTERVAL_SEC=600`.
