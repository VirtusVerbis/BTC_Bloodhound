...
- **Queue latch (moderate backlog):** when pending depth reaches `MAX_QUEUE_DEPTH` (360), `queueSchedulingPaused` blocks new crawl/maint/expand enqueue. Latch clears when depth drops to `QUEUE_SCHEDULING_RESUME_DEPTH` (180), not only at zero. `QUEUE_DRAIN_FIRST_DEPTH` (60) and `QUEUE_SOFT_THROTTLE_DEPTH` (120) gate drain-first ticks and crawl/poll enqueue throttling.
