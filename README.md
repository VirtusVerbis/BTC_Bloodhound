...
- **Log colorization:** set `INDEXER_LOG_COLOR=1` (Worker env / wrangler.toml) or run local indexer with `--log-color` to ANSI-colorize log prefixes and key labels (`id=`, `type=`, `address=`, `continuation=`, `error=`, `attempts=`, `reason=`, `duration=`, `queue=`) in `wrangler tail`.
