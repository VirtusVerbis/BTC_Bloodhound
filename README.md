...
**CF Workers tier quota presets:** Set `CF_WORKERS_TIER=free|paid` in `wrangler.toml` `[env.production.vars]` (and `config/sidecar.env` when running `--remote`). Tier selects defaults for cron quota pacing, D1 daily limits, and subrequest caps. Resolution order per setting: explicit env var (e.g. `D1_WRITE_DAILY_LIMIT`) → tier-specific cron var (`CRON_QUOTA_UTILIZATION_PCT_FREE` / `_PAID`) → built-in preset. `CRON_QUOTA_UTILIZATION_PCT` overrides both tier cron vars when set.
