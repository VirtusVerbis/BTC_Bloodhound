...
function baseConfig(): AppConfig {
  return {
    ...
    jobPruneIntervalDays: 5,
    jobPruneBatchSize: 500,
    jobPruneMaxBatchesPerRun: 20,
    jobPruneMaxDeletesPerRun: 10_000,
    jobPruneEnabled: true,
    jobCompletedAtBackfillBatchSize: 500,
    jobCompletedAtBackfillMaxBatchesPerTick: 5,
    rateLimitPruneInactiveDays: 7,
    rateLimitPruneBatchSize: 200,
    rateLimitPruneMaxBatchesPerRun: 10,
    syncStateOrphanPruneBatchSize: 500,
    syncStateOrphanPruneMaxBatchesPerRun: 10,
    maintenanceMaxBatchesPerTick: 25,
    maintenanceMaxWritesPerTick: 12_000,
  };
}
...
function mockStore(overrides: Partial<Store> = {}): Store {
  return {
    ...
    backfillDoneJobCompletedAt: vi.fn().mockResolvedValue({ affected: 0, batches: 0, complete: true }),
    seedLastCompletedJobCache: vi.fn().mockResolvedValue(false),
    countDoneJobsWithNullCompletedAt: vi.fn().mockResolvedValue(0),
    pruneDoneJobs: vi.fn().mockResolvedValue({ affected: 0, batches: 0, complete: true }),
...
describe("maintainOneHacker", () => {
  ...
  it("does not enqueue refresh_live_balance when poll is due", async () => {
    const addr = "bc1qhack";
    const store = mockStore({
      getAddress: vi.fn().mockResolvedValue({ address: addr, expandStatus: "expanded" }),
      getBackfillState: vi.fn().mockResolvedValue({
        payload: null,
        backfillComplete: true,
        lastBackfillAuditAt: new Date(ts).toISOString(),
        chainTxCountAtAudit: 10,
      }),
  ...
  it("does not enqueue refresh_live_balance when audit is due", async () => {
    const addr = "bc1qhack";
    const store = mockStore({
      getAddress: vi.fn().mockResolvedValue({ address: addr, expandStatus: "expanded" }),
      getBackfillState: vi.fn().mockResolvedValue({
      ...
      getSyncState: vi.fn().mockResolvedValue({ lastPolledAt: new Date(ts).toISOString() }),
    });
  ...
  it("enqueues refresh_live_balance for a backfilling hacker with stale balance", async () => {
    const addr = "bc1qhack";
    const store = mockStore({
      getAddress: vi.fn().mockResolvedValue({ address: addr, expandStatus: "backfilling" }),
      getBackfillState: vi.fn().mockResolvedValue({
...
describe("scheduleDownstreamCrawl", () => {
  ...
  it("calls listHackersCached once per tick", async () => {
    ...
    expect(store.refreshFlaggedHackersCache).not.toHaveBeenCalled();
