...
export type D1RowMeterOverhead = {
  reads: number;
  writes: number;
};
...
export class D1RowMeter {
  ...
  private overheadRowsRead = 0;
  private overheadRowsWritten = 0;
  ...
  rolloverIfNeeded(now = new Date()): boolean {
    ...
    this.rowsRead = 0;
    this.rowsWritten = 0;
    this.overheadRowsRead = 0;
    this.overheadRowsWritten = 0;
    for (const listener of this.rolloverListeners) listener();
    return true;
  }

  /** Route flush bookkeeping into overhead counters instead of app totals. */
  suppress(): void {
  ...
  drainOverhead(): D1RowMeterOverhead {
    const reads = this.overheadRowsRead;
    const writes = this.overheadRowsWritten;
    this.overheadRowsRead = 0;
    this.overheadRowsWritten = 0;
    return { reads, writes };
  }
  ...
  record(reads: number, writes: number, now = new Date()): boolean {
    ...
    if (this.suppressDepth > 0) {
      if (reads > 0) this.overheadRowsRead += reads;
      if (writes > 0) this.overheadRowsWritten += writes;
      return rolled;
    }
